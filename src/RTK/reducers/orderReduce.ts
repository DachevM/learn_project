...
export const orderSlice = createSlice({
    ...
    reducers: {
        ordersFilt(state, action: OrderAction) {
            state.orderFiltered = [...state.order].filter((el: IOrders) => el.user.name.toLowerCase().includes(action.payload.toLowerCase()))
        },
    ...
    extraReducers: {
        [fetchOrders.pending.type]: (state, action: OrderAction) => {
