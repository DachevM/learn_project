...
import {addCategory, removeSub} from "../../../Redux/action-creators/subcategoryAction";
...
const Subcategories = ({filtered}:SubcategoryProps) => {
    ...
    const newSubcategories = () => {
        ...
        if (newSubcategories.name.length !==0) dispatch(addCategory(newSubcategories))
        setCategName("")
    }
    ...
    return (
        <div className={"subcategories"}>
            <div className={"subcategories_head"}>
                <input value={name} onChange={(e) => setCategName(e.target.value)} type={"text"}
                       placeholder={"Введите название категории"} className={"categories_search_inp"}/>
                <button className={"categories_butt"} onClick={newSubcategories}>Добавить категорию</button>
            </div>
            <div>
                        ...
                        {filtered.map((elem) =>
                            <div key={elem.id} className={"categories_body_data"}>
                                ...
                                <div>
                                    ...
                                    <img className={"categories_trash"} src={Img.trash} alt={""} onClick={() => removeSubcategories(elem)}/>
                                </div>
                            </div>
                        )
