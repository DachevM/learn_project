...
const BannersList = ({banners,setBanners}:BanersBodyProps) => {
    const removeBanner = (banner:IBanners) => {
        return setBanners(banners.filter((e)=>e.id !== banner.id))
    }
    return (
        <div className={"banners_body"}>
            <div className={"banners_body_head"}>
                Заголовок
            </div>
                ...
                <div className={"banners_body_layout"}>
                    { banners.map((elem)=>
                        <div key={elem.id} className={"banners_body_banners"}>
                            <div className={"banners_body_right_banners"}>{elem.name}
                                <div><img className={"banners_pen"} src={Img.pen} alt={""} />
                                    <img className={"banners_trash"} src={Img.trash} alt={""} onClick={()=>removeBanner(elem)} /></div>
