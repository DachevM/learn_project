...
const SeminarsHistoryList = ({searchedSeminarsHistory, history, setHistory}:HistoryBodyProps) => {
    const removeSeminar = (seminar:IHistory) => {
        return setHistory(history.filter(e => e.id !== seminar.id))
    }
    ...
    return (
        <div className={"seminarsHistoryBody"}>
            ...
            <div className={"seminarsHistory_descr"}>
                <p className={"seminarsHistory_name"}><FormControlLabel label={""}
                                                                        control={<Checkbox onClick={checkAllHandler}
                                                                                           size={"small"}/>}/>Название
                    семинара</p>
                        ...
                        <>
                            {searchedSeminarsHistory.map((elem) =>
                                <div key={elem.id} className={"seminarsHistory_section"}>
                                    <div className={"seminarsHistory_name"}><FormControlLabel
                                        control={<Checkbox onChange={checkboxHandler}
                                                           checked={selectedItems.includes(elem.id)} value={elem.id}
                                                           size={"small"}/>} label={""}/>{elem.name}</div>
                                    <div className={"seminarsHistory_date"}>{elem.date}<FavoriteBorderOutlinedIcon
                                        sx={{color: pink[500]}}/> <DeleteOutlineOutlinedIcon
