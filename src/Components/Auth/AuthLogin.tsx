...
const AuthLogin = () => {
    ...
    const auth = () => {
        ...
        if (localStorage.getItem("E-Mail") === loginEmail && localStorage.getItem("password") === loginPass) {
            setIsAuth(true)
            navigate("/products")
        } else {
            ...
            <Box className={"login_box"}>
                ...
                <div className={"butt_box"}>
                    <ThemeProvider theme={theme}>
                        <Button onClick={auth} color="primary" variant="contained">Войти</Button></ThemeProvider>
                    <Link className={"link_to_auth"} to={"/auth/register"}> У меня еще нет аккаунта</Link>
                </div>
            </Box>
        </div>
    );
