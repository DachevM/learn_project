...
const Pagination = ({pages,total}:IPaginationProps) => {
    ...
    return (
        <div className={"pagination_pages"}>
            Показывать
            <select onClick={limitChange} onChange={(e:any)=> setMenu(e.target.value)} className="menu">
                <option value={5} >5</option>
                <option value={10} >10</option>
                <option value={15} >15</option>
            </select>
            Страница
            <button disabled={true} className={"pagination_pages_butt"}>{pages}</button>
            <div className={"total_pages"}>из {total()}</div>
            <button disabled={pages <= 1} onClick={pagesMinus} className={"pagination_pages_arrow1"}>
                <KeyboardArrowLeftOutlinedIcon fontSize={"small"}/>
            </button>
            <button disabled={pages === total()} onClick={pagesPlus} className={"pagination_pages_arrow2"}>
